...
setTimeout(() =>{
    ...
    .then(res => res.json)
    .then(data=> {
        localStorage('pokemonData', JSON.stringify(data.name))
        localStorage('pokemonImage', data.sprites.front_default);

        const h = document.getElementById('title');
        h = JSON.stringify(data.name)
        const i = document.getElementById('image');
        i = data.sprites.front_default
    .catch(err => console.error('loi roi'));
    },3000)
})
